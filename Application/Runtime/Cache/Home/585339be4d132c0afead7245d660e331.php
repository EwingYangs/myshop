...
<link rel="stylesheet" href="/Public/Home/css/fillin.css" type="text/css">
<script type="text/javascript" src="/Public/Home/js/cart2.js"></script>
	...
	<div class="fillin w990 bc mt15">
		...
		<div class="fillin_bd">
			...
			<div class="address">
				...
				<div class="address_select none">
					...
					<input type="hidden" name="ab" value="add">
						<ul>
							<li>
							...
							<li>
								...
								<select name="shr_province" id="province" onchange="showcity()">

								</select>

								<select name="shr_city"id="city" onchange="showstrict()">
									
								</select>

								<select name="shr_area" id="district">
			...
			<?php foreach($data as $k=>$v){?>
				<tr>
					<td class="col1">
					<a href=""><?php showImage($v['mid_logo'])?></a>  
		...
		<div class="fillin_ft">
			<a onclick="document.address_form.submit()" href="javascript:#"><span>提交订单</span></a>
			<p>应付总额：<strong>￥<?php echo sprintf("%.2f", $total);?>元</strong></p>
 ...
 function f1(){
    $.get('/xml/ChinaArea.xml',function(msg){
      //alert(msg);
      //console.log(msg.getElementsByTagName('province'));
      //console.log($(document).find('option'));
      //console.log($('option'));
      //把请求回来的xml信息保存到全局变量中
      xmldom = msg;
      ...
      pros.each(function(k,v){
        ...
        $('#province').append("<option id='"+id+"'value='"+nm+"'>"+nm+"</option>");
 ...
 function showcity(pid){
      ...
      citys.each(function(k,v){
          ...
          $('#city').append("<option id='"+id+"'value='"+nm+"'>"+nm+"</option>");
      });
 }
 function showstrict(pid){
      if(pid == null){
